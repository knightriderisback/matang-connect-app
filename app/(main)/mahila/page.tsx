...
const TYPES = ["resource", "event", "success_story", "scheme"];
    ...
    <div className="p-4 space-y-4 pb-24">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-matang-navy">{t("mahila.title")}</h1>
        </div>
        {isStaff && (
      ...
      <p className="text-xs text-gray-500">{t("mahila.subtitle")}</p>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Flower2}
          title={t("mahila.noGroups")}
          description={t("mahila.subtitle")}
          ...
          <Card key={p.id}>
            <CardContent className="p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                {p.event_date && <span>📅 {timeAgo(p.event_date)}</span>}
                {p.contact_phone && (
