    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-matang-navy">{t("jobs.title") || "Jobs & Livelihood"}</h1>
        </div>
        {isStaff && (
          <Button className="text-sm px-3 py-1.5" onClick={() => setShowForm(!showForm)}>
            <Plus size={16} /> {t("jobs.postJob") || "Post"}
          </Button>
        )}
      </div>
      ...
      {showForm && (
        <Card className="border-matang-gold/30">
          <CardContent className="p-4 space-y-3">
            ...
            />
            <Input
              label={t("auth.phone") || "Contact Phone"}
              type="tel"
              value={form.contact_phone}
              onChange={(e) => setForm({ ...form, contact_phone: e.target.value })}
            />
            <Input
              label={t("jobs.salary") || "Salary Range"}
